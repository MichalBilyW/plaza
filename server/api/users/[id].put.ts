...
import { defineApiHandler, createNotFoundError, createValidationError, ApiError, ErrorCodes } from '@/server/utils/errors'
...
export default defineEventHandler(
  defineApiHandler(async (event) => {
    ...
    if (updateData.email && updateData.email !== user.email) {
      ...
      if (existingUser) {
        throw new ApiError(ErrorCodes.VALIDATION_ERROR, 'Uživatel s tímto emailem již existuje', 400)
      }
    }
