...
export default defineEventHandler(
  defineApiHandler(async (event) => {
    ...
    if (!session) {
      ...
      throw new ApiError(ErrorCodes.UNAUTHORIZED, 'Neplatný nebo expirovaný refresh token', 401)
    }
    ...
    if (!user) {
      ...
      await session.save()
      clearAuthCookies(event)
      throw new ApiError(ErrorCodes.UNAUTHORIZED, 'Uživatel neexistuje nebo je deaktivován', 401)
    }
