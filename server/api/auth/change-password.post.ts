...
import { defineApiHandler, createValidationError, ApiError, ErrorCodes } from '@/server/utils/errors'
  ...
  defineApiHandler(async (event) => {
    ...
    if (!result.success) {
      throw createValidationError('Neplatné údaje', {
        currentPassword: result.error.issues.find((i) => i.path[0] === 'currentPassword')?.message,
        newPassword: result.error.issues.find((i) => i.path[0] === 'newPassword')?.message,
        confirmPassword: result.error.issues.find((i) => i.path[0] === 'confirmPassword')?.message,
      })
    }
