<template>
  <div class="py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <!-- Header -->
      <div class="mb-8">
        <h1 class="text-3xl font-bold mb-2">Obchody a služby</h1>
        <p class="text-gray-600">
          Objevte všechny obchody a služby v OC Plaza Liberec
        </p>
      </div>
      ...
      <div class="flex flex-col md:flex-row gap-4 mb-8">
        ...
        <select
          ...
          class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-plaza-500 focus:border-plaza-500"
        >
      ...
      <div class="flex flex-wrap gap-2 mb-8">
        ...
        <button
          ...
          class="px-4 py-2 rounded-full text-sm font-medium transition-colors"
          :class="selectedCategory === cat._id
            ? 'bg-plaza-600 text-white'
            : 'bg-gray-100 text-gray-700 hover:bg-gray-200'"
        >
          {{ cat.name }}
      ...
      <div v-else-if="shops.length === 0" class="text-center py-16">
        ...
        <h3 class="text-lg font-medium text-gray-900 mb-1">Žádné obchody nenalezeny</h3>
        <p class="text-gray-500">Zkuste upravit filtry nebo vyhledávání</p>
      </div>
...
// Load categories
const { data: categories } = await useFetch<Array<{ _id: string; name: string; slug: string; shopCount: number }>>('/api/categories')
