			...
			<template #item="{ element: item }">
				<div class="bg-white rounded-xl shadow-sm overflow-hidden group relative">
					...
					<div class="aspect-square relative overflow-hidden bg-gray-100">
						...
						<!-- Display until badge -->
						<div
							v-if="item.displayUntil"
							class="absolute bottom-2 left-2 right-2 flex justify-center"
							:title="
								isExpired(item.displayUntil)
									? t('cms.news.expiredOn', { date: formatShortDate(item.displayUntil) })
									: t('cms.news.displayUntilOn', {
											date: formatShortDate(item.displayUntil),
										})
							"
						>
							<span
							...
							>
								...
								<span>
									{{ isExpired(item.displayUntil) ? t('cms.news.expired') : t('cms.news.until') }}
									{{ formatShortDate(item.displayUntil) }}
								</span>
							</span>
						</div>
					</div>
