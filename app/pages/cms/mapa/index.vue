          ...
          <div class="space-y-4">
            ...
            <div v-if="selectedUnit.shop">
              ...
              <div class="flex items-center gap-3 p-3 bg-green-50 rounded-lg">
                ...
                >
                  <img :src="selectedUnit.shop.logo" :alt="selectedUnit.shop.name" class="w-full h-full object-contain" />
                </div>
                <div v-else class="w-10 h-10 rounded-lg bg-gray-200 flex items-center justify-center">
                  <span class="text-gray-500 text-lg font-bold">{{ selectedUnit.shop.name.charAt(0) }}</span>
                </div>
                <div>
                  <p class="font-medium text-gray-900">{{ selectedUnit.shop.name }}</p>
                  <span
                    :class="selectedUnit.shop.isActive ? 'text-green-600' : 'text-orange-600'"
                    class="text-xs"
                  >
                    {{ selectedUnit.shop.isActive ? t('cms.shops.active') : t('cms.shops.inactive') }}
                  </span>
                </div>
              </div>
              <NuxtLink
            ...
            <div v-else>
              ...
              <div v-if="availableShops.length === 0" class="p-4 bg-gray-50 rounded-lg text-center">
                <p class="text-gray-500 text-sm">{{ t('cms.map.noShopsToAssign') }}</p>
                <NuxtLink
                  to="/cms/obchody/novy"
                  class="inline-flex items-center gap-1 mt-2 text-sm text-indigo-600 hover:text-indigo-700"
                >
...
// Fetch map units
const { data: mapData, pending, error, refresh } = await useFetch<MapUnitsResponse>('/api/map/units')
