<template>
	<div class="p-4 sm:p-6 lg:p-8">
		...
		>
			<div>
				...
				<p class="text-plaza-dark mt-1 text-sm sm:text-base">
					{{ t('cms.events.subtitle') }}
				</p>
			</div>
			<NuxtLink
				to="/cms/akce/nova"
				class="inline-flex items-center justify-center gap-2 px-4 py-2 bg-cms-events-600 text-white rounded-lg hover:bg-cms-events-700 transition-colors text-sm sm:text-base"
			>
		...
		<div class="mb-6 flex flex-col sm:flex-row gap-4">
			<div class="flex-1">
				<input
					v-model="search"
					type="text"
					:placeholder="t('cms.events.searchPlaceholder')"
					class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cms-events-500 focus:border-transparent"
				/>
			</div>
			<select
				v-model="shopFilter"
				class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cms-events-500 focus:border-transparent"
			>
				<option value="">{{ t('cms.events.allShops') }}</option>
			...
			<select
				...
				class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-cms-events-500 focus:border-transparent"
			>
				<option value="">{{ t('cms.events.allStatus') }}</option>
				<option value="true">{{ t('cms.events.activeOnly') }}</option>
				<option value="false">{{ t('cms.events.inactiveOnly') }}</option>
			</select>
		</div>
		...
		<!-- Empty state -->
		<div v-else-if="!events.length" class="bg-white rounded-xl shadow-sm p-8 text-center">
			<svg
			...
			<p class="text-plaza-dark">{{ t('cms.events.noEvents') }}</p>
		</div>
			...
			>
				<div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
					<h3 class="text-lg font-semibold text-gray-900 mb-2">
						{{ t('cms.events.deleteConfirmTitle') }}
					</h3>
					<p class="text-plaza-dark mb-6">
						{{
							t('cms.events.deleteConfirmMessage', { name: deleteModal.event?.name })
						}}
					</p>
...
const {
	...
	query: computed(() => ({
		...
		shopId: shopFilter.value || undefined,
...
const deleteEvent = async () => {
	...
	try {
		await secureFetch(`/api/events/${deleteModal.event._id}`, {
			method: 'DELETE',
		})
		flash.success(t('cms.events.deleteSuccess'))
		deleteModal.show = false
		await refresh()
	} catch {
		flash.error(t('cms.events.deleteError'))
	} finally {
