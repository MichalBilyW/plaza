  ...
  <div class="p-4 sm:p-6 lg:p-8">
    <div class="mb-8">
      ...
      <div class="flex items-center justify-between">
        <h1 class="text-xl sm:text-2xl font-bold text-gray-900">{{ t('cms.shops.editShop') }}</h1>
        <NuxtLink
          v-if="shop"
    ...
    <!-- Error state -->
    <div v-else-if="fetchError" class="bg-white rounded-xl shadow-sm p-8 text-center text-red-500">
      {{ t('common.error') }}: {{ fetchError.message }}
    </div>

    <!-- General error -->
    <div v-if="generalError && !fetchError" class="max-w-4xl mb-4 p-4 bg-red-50 border border-red-200 rounded-lg">
    ...
    <form v-if="shop && !fetchError" @submit.prevent="handleSubmit" class="space-y-8">
      ...
      <div class="bg-white rounded-xl shadow-sm p-6">
        <h2 class="text-lg font-semibold mb-6 text-cms-shops-700 border-b border-cms-shops-100 pb-2">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div class="lg:col-span-2">
            ...
            />
            <p class="mt-1 text-xs text-gray-500">{{ form.shortDescription?.length || 0 }}/300</p>
          </div>
      ...
      <div class="bg-white rounded-xl shadow-sm p-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div>
            ...
            >
              <option value="">{{ form.floorId ? t('cms.shops.selectUnit') : t('cms.shops.selectFloorFirst') }}</option>
              <option
              ...
              >
                {{ unit.id }} - {{ unit.label }}{{ unit.isOccupied && unit.id !== form.unitCode ? ` (${unit.shopName})` : '' }}
              </option>
            </select>
      ...
      <div class="bg-white rounded-xl shadow-sm p-6">
        ...
        <!-- Sociální sítě -->
        <div class="mt-6">
          <h3 class="text-sm font-medium text-gray-700 mb-3">{{ t('cms.shops.socialLinks') }}</h3>
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div>
              <label for="facebook" class="block text-xs text-gray-500 mb-1">Facebook</label>
              <input
                id="facebook"
            ...
            <div>
              <label for="instagram" class="block text-xs text-gray-500 mb-1">Instagram</label>
              <input
                id="instagram"
            ...
            <div>
              <label for="twitter" class="block text-xs text-gray-500 mb-1">Twitter / X</label>
              <input
                id="twitter"
      ...
      <div class="bg-white rounded-xl shadow-sm p-6">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-lg font-semibold text-cms-shops-700 border-b border-cms-shops-100 pb-2 flex-1">
        ...
        <div v-if="form.specialOpeningHours.length === 0" class="text-center py-6 text-gray-500">
          ...
          >
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
              ...
              <div v-if="entry.type === 'single'">
                <label class="block text-xs text-gray-500 mb-1">{{ t('cms.shops.date') }}</label>
                <input
                  v-model="entry.date"
                  type="date"
              ...
              <template v-else>
                <div>
                  <label class="block text-xs text-gray-500 mb-1">{{ t('cms.shops.dateFrom') }}</label>
                  <input
                ...
                <div>
                  <label class="block text-xs text-gray-500 mb-1">{{ t('cms.shops.dateTo') }}</label>
                  <input
              ...
              <template v-if="!entry.closed">
                <div class="flex items-end gap-2">
                  <div>
                    <label class="block text-xs text-gray-500 mb-1">{{ t('cms.shops.openTime') }}</label>
                    <input
                      v-model="entry.open"
                      type="time"
                      class="px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-cms-shops-500"
                  ...
                  <div>
                    <label class="block text-xs text-gray-500 mb-1">{{ t('cms.shops.closeTime') }}</label>
                    <input
                      v-model="entry.close"
                      type="time"
                      class="px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-cms-shops-500"
            ...
            <!-- Poznámka -->
            <div class="mt-3">
              <label class="block text-xs text-gray-500 mb-1">{{ t('cms.shops.specialNote') }}</label>
              <input
                v-model="entry.note"
                type="text"
                maxlength="200"
      ...
      <div class="bg-white rounded-xl shadow-sm p-6">
        ...
        <div class="space-y-6">
          <div>
            ...
            />
            <p class="mt-1 text-xs text-gray-500">{{ form.seoTitle?.length || 0 }}/60</p>
          </div>

          <div>
            <label for="seoDescription" class="block text-sm font-medium text-gray-700 mb-1">
            ...
            />
            <p class="mt-1 text-xs text-gray-500">{{ form.seoDescription?.length || 0 }}/160</p>
          </div>
        </div>
      </div>
...
// Fetch shop data
const { data: shop, pending, error: fetchError, refresh } = await useFetch<Shop>(`/api/shops/${shopId}`)

// Dynamic SEO title
const seoTitle = computed(() => shop.value?.name ? `${t('cms.shops.editShop')}: ${shop.value.name}` : t('cms.shops.editShop'))
watchEffect(() => {
  usePlazaSeo({
...
watch(shop, (newShop) => {
  if (newShop) {
    ...
    const existingHours = new Map((newShop.openingHours || []).map((h: OpeningHoursEntry) => [h.day, h]))
...
const { errors, generalError, clearErrors, handleApiError, hasError, getError, scrollToFirstError } = useFormErrors()
...
watch(() => form.floorId, (newFloorId, oldFloorId) => {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const specialOpeningHours = form.specialOpeningHours
      .filter(entry => entry.type === 'single' ? entry.date : (entry.dateFrom && entry.dateTo))
      .map(entry => ({
