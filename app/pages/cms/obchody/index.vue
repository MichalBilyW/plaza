			...
			<table class="w-full min-w-[800px]">
				<thead class="bg-gray-50 border-b">
					<tr>
						...
						<CmsSortableHeader
							field="unitCode"
							:sort-icon="getSortIcon('unitCode')"
							class="w-20"
							@sort="toggleSort"
						>
							{{ t('cms.shops.units') }}
						...
						<CmsSortableHeader
							field="updatedAt"
							:sort-icon="getSortIcon('updatedAt')"
							class="w-28"
							@sort="toggleSort"
						>
					...
					<tr v-for="shop in sortedShops" :key="shop._id" class="hover:bg-gray-50">
						<td class="px-4 py-3">
							<div class="flex items-center gap-3">
								<div
									class="w-9 h-9 rounded-lg bg-gray-100 flex items-center justify-center overflow-hidden flex-shrink-0"
								>
									<img
										v-if="shop.logo"
										:src="shop.logo"
								...
								<div class="min-w-0">
									<span
										class="font-medium text-gray-900 block truncate max-w-[200px]"
										>{{ shop.name }}</span
									>
...
const {
	...
	error,
	refresh,
} = await useFetch<PaginatedResponse<Shop>>('/api/shops', {
	query: computed(() => ({
		page: page.value,
		limit: 20,
