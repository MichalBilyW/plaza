<template>
  <div class="p-4 sm:p-6 lg:p-8">
    <div class="mb-8">
      ...
      <h1 class="text-xl sm:text-2xl font-bold text-gray-900">{{ t('cms.categories.editCategory') }}</h1>
    </div>
    ...
    <!-- General error -->
    <div v-if="generalError && !fetchError" class="max-w-2xl mb-4 p-4 bg-red-50 border border-red-200 rounded-lg">
      <p class="text-sm text-red-600">{{ generalError }}</p>
    </div>

    <form v-if="category && !fetchError" @submit.prevent="handleSubmit" class="max-w-2xl space-y-6">
      <div class="bg-white rounded-xl shadow-sm p-6">
        <h2 class="text-lg font-semibold mb-6 text-cms-categories-700 border-b border-cms-categories-100 pb-2">
        ...
        <div class="space-y-6">
          ...
          <div>
            <label class="inline-flex items-center gap-3 cursor-pointer">
              ...
              />
              <span class="text-sm font-medium text-gray-700">{{ t('cms.categories.isActive') }}</span>
            </label>
          </div>
        </div>
      </div>
    ...
    <Teleport to="body">
      <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
        <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
          <h3 class="text-lg font-semibold mb-2">{{ t('cms.categories.deleteConfirmTitle') }}</h3>
...
// Fetch category data
const { data: category, pending, error: fetchError } = await useFetch<Category>(`/api/categories/${categoryId}`)

// Dynamic SEO title
const seoTitle = computed(() => category.value?.name ? `${t('cms.categories.editCategory')}: ${category.value.name}` : t('cms.categories.editCategory'))
watchEffect(() => {
  usePlazaSeo({
