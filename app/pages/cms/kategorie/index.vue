<template>
  <div class="p-4 sm:p-6 lg:p-8">
    <div class="mb-6 sm:mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 class="text-xl sm:text-2xl font-bold text-gray-900">{{ t('cms.categories.title') }}</h1>
        <p class="text-gray-500 mt-1 text-sm sm:text-base">{{ t('cms.categories.subtitle') }}</p>
      </div>
      <NuxtLink
    ...
    <div v-else class="hidden lg:block bg-white rounded-xl shadow-sm overflow-x-auto">
      <table class="w-full min-w-[600px]">
        <thead class="bg-gray-50 border-b">
          <tr>
            <CmsSortableHeader field="sortOrder" :sort-icon="getSortIcon('sortOrder')" class="w-20" @sort="toggleSort">{{ t('cms.categories.order') }}</CmsSortableHeader>
            <CmsSortableHeader field="name" :sort-icon="getSortIcon('name')" @sort="toggleSort">{{ t('cms.categories.name') }}</CmsSortableHeader>
            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ t('cms.categories.shopCount') }}</th>
            <CmsSortableHeader field="isActive" :sort-icon="getSortIcon('isActive')" class="w-24" @sort="toggleSort">{{ t('cms.categories.status') }}</CmsSortableHeader>
            <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider w-24">{{ t('common.actions') }}</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-200">
          <tr v-for="category in sortedCategories" :key="category._id" class="hover:bg-gray-50">
            <td class="px-4 py-3">
              <span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-cms-categories-100 text-cms-categories-800 font-medium">
                {{ category.sortOrder }}
              </span>
            </td>
            <td class="px-4 py-3">
              <NuxtLink :to="`/cms/kategorie/${category._id}`" class="text-gray-900 hover:text-cms-categories-600 font-medium">
                {{ category.name }}
              </NuxtLink>
            ...
            <td class="px-4 py-3 text-right">
              <div class="flex items-center justify-end gap-1">
                ...
                <button
                  ...
                  :title="(category.shopCount || 0) > 0 ? t('cms.categories.cannotDeleteWithShops') : t('common.delete')"
                >
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      >
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-sm text-gray-500 truncate mt-0.5">{{ category.shopCount || 0 }} {{ t('cms.categories.shops') }}</p>
          </div>
    ...
    <div v-if="deleteModal.show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div class="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-2">{{ t('cms.categories.deleteConfirmTitle') }}</h3>
        <p class="text-gray-500 mb-6">{{ t('cms.categories.deleteConfirmMessage', { name: deleteModal.category?.name }) }}</p>
...
// Load categories
const { data: categoriesData, pending, error, refresh } = await useFetch<PaginatedResponse<Category>>('/api/categories', {
  query: { limit: 100 }
})
