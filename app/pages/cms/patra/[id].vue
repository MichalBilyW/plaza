<template>
  <div class="p-4 sm:p-6 lg:p-8">
    <div class="mb-8">
      ...
      <h1 class="text-xl sm:text-2xl font-bold text-gray-900">{{ t('cms.floors.editFloor') }}</h1>
    </div>
    ...
    <!-- General error -->
    <div v-if="generalError" class="max-w-2xl mb-4 p-4 bg-red-50 border border-red-200 rounded-lg">
    ...
    <form v-else-if="floor" @submit.prevent="handleSubmit" class="max-w-2xl space-y-6">
      <div class="bg-white rounded-xl shadow-sm p-6">
        <h2 class="text-lg font-semibold mb-6 text-indigo-700 border-b border-indigo-100 pb-2">
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              type="text"
              required
              :class="['w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent', hasError('name') ? 'border-red-500 bg-red-50' : 'border-gray-300']"
              :placeholder="t('cms.floors.namePlaceholder')"
              @input="errors.name = undefined"
            />
            <p v-if="hasError('name')" class="mt-1 text-sm text-red-600">{{ getError('name') }}</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              v-model="form.slug"
              type="text"
              :class="['w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent', hasError('slug') ? 'border-red-500 bg-red-50' : 'border-gray-300']"
              :placeholder="t('cms.floors.slugPlaceholder')"
              @input="errors.slug = undefined"
            />
            <p v-if="hasError('slug')" class="mt-1 text-sm text-red-600">{{ getError('slug') }}</p>
            <p v-else class="mt-1 text-xs text-gray-500">{{ t('cms.floors.slugHint') }}</p>
          </div>
          ...
          <div>
            ...
            <input
              ...
              type="number"
              required
              :class="['w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent', hasError('level') ? 'border-red-500 bg-red-50' : 'border-gray-300']"
              :placeholder="t('cms.floors.levelPlaceholder')"
              @input="errors.level = undefined"
            />
            <p v-if="hasError('level')" class="mt-1 text-sm text-red-600">{{ getError('level') }}</p>
            <p v-else class="mt-1 text-xs text-gray-500">{{ t('cms.floors.levelHint') }}</p>
          </div>
