...
// Load floor data
const { data: floor, pending, error: fetchError, refresh } = await useFetch<Floor>(`/api/floors/${floorId}`)

// SVG mapa - sledovat aktuální stav
const currentSvgMap = computed(() => floor.value?.svgMap || null)
...
const handleSvgUpload = async (event: Event) => {
	...
	try {
		...
		// Update floor with new SVG
		await secureFetch(`/api/floors/${floorId}`, {
			method: 'PUT',
			body: { svgMap: uploadResponse.url },
		})
...
const removeSvgMap = async () => {
	...
	try {
		...
		flash.error(t('cms.floors.svgRemoveError'))
		console.error('Failed to remove SVG:', e)
	}
}
</script>
