    ...
    <div v-else class="bg-white rounded-xl shadow-sm p-6 max-w-2xl">
      <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div class="pt-4 border-t text-sm text-gray-500">
          <p v-if="userData?.createdAt">{{ $t('cms.users.createdAt') }}: {{ formatDate(userData.createdAt) }}</p>
          <p v-if="userData?.lastLoginAt">{{ $t('cms.users.lastLogin') }}: {{ formatDate(userData.lastLoginAt) }}</p>
        </div>
...
// Načíst uživatele
const { data: userData, pending, error } = await useFetch<User>(`/api/users/${userId}`, {
  credentials: 'include'
})
