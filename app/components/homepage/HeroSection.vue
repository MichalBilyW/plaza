<template>
	<div class="bg-gradient-to-b from-[#131313] to-[#1A1A1A] pt-[60px] lg:pt-[90px]">
		<!-- hero bg -->
		<div
			class="max-md:hidden z-0 absolute top-0 left-0 w-full h-[450px] bg-gradient-to-b from-[#131313] to-[#1A1A1A]"
		></div>
		...
		<section class="z-10 container relative transparent overflow-hidden" aria-label="Hero">
			...
			<div
				v-if="showBorder"
				class="absolute top-0 left-0 w-[calc(100%-16px)] h-[calc(240px-16px)] sm:w-[calc(100%-24px)] sm:h-[calc(320px-24px)] md:w-[calc(100%-32px)] md:h-[calc(400px-32px)] bg-transparent m-2 sm:m-3 md:m-4 border sm:border md:border-2 border-white/70 rounded-[5px_20px_5px_5px] z-10 pointer-events-none"
				aria-hidden="true"
			></div>
