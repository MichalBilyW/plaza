    ...
    >
      ...
      <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-800 w-64">
        <div class="flex items-center gap-3">
          <NuxtLink to="/cms/ucet" class="w-8 h-8 rounded-full bg-plaza-600 flex items-center justify-center text-sm font-medium hover:bg-plaza-500 transition-colors" @click="sidebarOpen = false">
            {{ user?.name?.charAt(0)?.toUpperCase() || '?' }}
          </NuxtLink>
          <NuxtLink to="/cms/ucet" class="flex-1 min-w-0 hover:opacity-80 transition-opacity" @click="sidebarOpen = false">
            <p class="text-sm font-medium truncate">{{ user?.name }}</p>
            <p class="text-xs text-gray-400">{{ user?.role === 'admin' ? $t('cms.roles.admin') : $t('cms.roles.editor') }}</p>
          </NuxtLink>
          <button
            @click="logout"
            class="p-2 hover:bg-gray-800 rounded-lg transition-colors"
