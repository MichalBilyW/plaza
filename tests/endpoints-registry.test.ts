...
describe('Endpoints Registry Consistency', () => {
    ...
    for (const endpoint of registeredEndpoints) {
      ...
      const found = serverRouteFiles.some(file => {
        ...
        return expectedFiles.some(expected =>
          normalizedFile === expected.replace(/\\/g, '/')
        )
      })

      if (!found) {
        missingFiles.push(`${endpoint.method} ${endpoint.path} -> ${expectedFiles.join(' nebo ')}`)
      }
    }
  ...
  it('should have all implemented endpoints registered in endpoints registry', () => {
    ...
    for (const file of serverRouteFiles) {
      ...
      const found = registeredEndpoints.some(e =>
    ...
    expect(unregisteredEndpoints, `Neregistrované endpointy: ${unregisteredEndpoints.join(', ')}`).toHaveLength(0)
  })
